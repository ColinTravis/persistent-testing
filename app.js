...
console.log(socket.id); //prints out socket that connected. (ie. all users + the projection)
  ...
  socket.on('addRectangle', function(data){ //look for any messages with the 'addRectangle'
    ...
    io.emit('projectionRectangle', data); //If socket receives "addrectangle" then send "projectionRectangle". send out a message to the projection to add a rectangle to the screen.
